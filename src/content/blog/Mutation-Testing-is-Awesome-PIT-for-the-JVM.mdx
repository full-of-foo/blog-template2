...
So now lets gets webscale and get our candidate unit tested and mutation tested under Maven. The _pitest_ artifact and its related dependencies are hosted by Sonatype so we can add the typical [`oss-sonatype`](https://github.com/full-of-foo/pit-coverage-tool-review/blob/master/practical/maven-pit-review/pom.xml#L9) _repository_ and _pluginRepository_ entries to our _pom.xml_. We can then simply configure PIT under JUnit4 as follows:
